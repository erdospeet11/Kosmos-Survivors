...
var OptionSlot = preload("res://option_slot.tscn")
...
func get_availabel_resource_in(items) -> Array[Item]:
...
func show_option():
	var weapons_available = get_availabel_resource_in(weapons)
	var passive_item_available = get_availabel_resource_in(passive_items)
...
func get_available_upgrades() -> Array[Item]:
	...
	for weapon : Weapon in get_availabel_resource_in(weapons):
		if weapon.is_upgradable():
			upgrades.append(weapon)
	
	for passive_item : PassiveItem in get_availabel_resource_in(passive_items):
